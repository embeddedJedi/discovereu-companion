...
- **v1.7 Green Hostel + Carbon Offset** (2026-04-13):
  - `data/green-hostels.json` — 12 entries across 10 cities (6 Green Key/EU Ecolabel/Biosphere verified: Paris×2, Barcelona×2, Copenhagen, Stockholm; 6 placeholder `verify:true` entries for Berlin/Amsterdam/Madrid/Vienna/Prague/Lisbon — editorial refresh pending)
  - `state.filters.greenHostelsOnly` boolean + migrate
  - `js/map/green-hostels-layer.js` — Leaflet layer with 🌱 markers, cert-badge popups, state-driven enable/disable
  - `js/ui/offset-cta.js` — reusable "Balance your trip" card with 3 non-affiliate provider links (myclimate / ClimateCare / Trees for All); rendered in Impact Dashboard when CO₂ > 0
  - Toggle integrated in `js/pages/map.js` layer-toggles strip (alongside wheelchair + pickpocket)
  - `css/green-hostel.css` (minimal styling — full polish deferred)
  - 25 i18n leaves (green.* 17 + offset.* 8) en+tr; de/fr/es/it deferred
  - PWA cache v10 → v11
  - 6 commits
  - Spec: `docs/superpowers/specs/2026-04-13-green-hostel-offset-design.md`
